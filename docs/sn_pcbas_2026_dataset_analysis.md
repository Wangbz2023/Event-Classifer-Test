...
当前仓库已经收束为 PCABS2026 单数据集链路，建议使用单独根目录：
...
├── extracted/
│   └── VAL/
│       └── val_tactical_data.h5
├── processed/
│   └── <full_exports>.jsonl
└── samples/
    └── <small_examples>.jsonl
```
...
- `raw/`、`extracted/`、`processed/` 都是本地大文件区，不入库；`samples/` 只放小型可读样例。

## 6. 基于真实 `val_tactical_data.h5` 的结构验证

本地真实文件 [val_tactical_data.h5](D:\Code\Event-Classifer-Test\data\pcbas2026\extracted\VAL\val_tactical_data.h5) 已经可以确认 `SN-PCBAS-2026` 的一个关键事实：解压后的 tactical annotations 主体是 **HDF5 文件**，而不是单层 JSON。
...
path = r"D:\Code\Event-Classifer-Test\data\pcbas2026\extracted\VAL\val_tactical_data.h5"
...
当前仓库执行链路已经收束到 PCABS2026，不再保留旧数据集下载脚本。历史 SoccerNet/GSR/Action Spotting 分析文档仍作为备用研究材料保留，但当前主链路只依赖：

- `scripts/download_sn_pcbas.py`：从 Hugging Face gated dataset 下载 split 级 tactical zip
- `scripts/inspect_sn_pcbas.py`：检查 zip 结构，并按 split / game query 抽取
- `scripts/inspect_sn_pcbas_h5.py`：读取 HDF5，导出 `jsonl/csv/json` 事件样例或全量解析结果
...
- 若以后重新引入 GSR/Tracking/SynLoc，也应作为独立对照链路接回，而不是混进当前 PCABS2026 主数据目录
